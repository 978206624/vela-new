...
export default function ChapterCreationDialog({ isOpen, onClose, prefill }: Props) {
  ...
  const [preview, setPreview] = useState<{ chapterInfo: ChapterInfo; ctx: ChapterContextResult } | null>(null)
  ...
  const handleStart = async () => {
    ...
    setAssembling(true)
    try {
      const ctx = await buildChapterContext(chapterInfo)
      setPreview({ chapterInfo, ctx })
    } catch (e) {
      toast.error(`上下文拼装失败：${e instanceof Error ? e.message : String(e)}`)
    } finally {
      setAssembling(false)
    }
  }
  ...
  const handleOpenChange = (open: boolean) => {
    // 生成中或预览未决时，禁止通过点击外部或 ESC 关闭
    if (!open && !isChapterRunning && !preview) onClose()
  }
  ...
  return (
        ...
        <div className="px-5 py-4 space-y-3">
            ...
            <DialogFooter className="sm:justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="ai" size="lg" onClick={handleStart} disabled={isChapterRunning || assembling}>
                  {isChapterRunning ? (
    ...
    {/* 上下文预览 / Prompt 组装确认（设计屏 14）—— 确认后才真正发送 */}
    {preview && (
      <ContextPreview
        open={true}
        chapterNumber={preview.chapterInfo.chapterNumber}
        segments={preview.ctx.segments}
        estimatedTokens={preview.ctx.estimatedTokens}
        tokenBudget={preview.ctx.tokenBudget}
