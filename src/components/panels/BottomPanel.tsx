...
import { loadLLMData, purposeLabel, type LLMStats, type LLMCallRecord } from '../../services/stats-service'
...
function ModelsView() {
  ...
  const [history, setHistory] = useState<LLMCallRecord[]>([])

  const loadData = useCallback(async () => {
    try {
      const { stats: s, history: h } = await loadLLMData(50)
      setStats(s)
      setHistory(h)
    } catch { /* 忽略 */ }
  }, [])
  ...
  useEffect(() => {
    loadData()
    // 每次 LLM 调用写入 llm_calls 后自动刷新（base-command 在 onDone/onError 后 emit）
    const unsub = globalEventBus.on('LLM_CALL_LOGGED', () => { loadData() })
    return unsub
  }, [loadData])
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      {/* 顶部：今日汇总（设计屏 25 右上） */}
      <div
        className="flex items-center justify-end px-4 py-1.5 flex-shrink-0 text-[0.7rem]"
        style={{ borderBottom: '1px solid var(--color-border)', color: 'var(--color-text-muted)' }}
      >
        今日 <span className="font-semibold text-[var(--color-text-secondary)] mx-1">{stats?.todayCalls ?? 0}</span> 次调用
        <span className="mx-1.5 opacity-50">·</span>
        <span className="font-semibold font-mono text-[var(--color-text-secondary)]">{(stats?.todayTokens ?? 0).toLocaleString()}</span> tokens
      </div>
            ...
            <tbody>
              {history.map((row) => (
                <tr
                  ...
                  style={{ borderBottom: '1px solid var(--color-border)' }}
                >
                  <td className="px-4 py-1 font-mono text-[var(--color-text-muted)]">
                    {new Date(row.createdAt).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}
                  </td>
                  <td className="px-2 py-1 text-[var(--color-text-secondary)]">{purposeLabel(row.purpose)}</td>
                  <td className="px-2 py-1 text-[var(--color-text-secondary)]">{row.modelName || '-'}</td>
                  <td className="px-2 py-1 text-right font-mono text-[var(--color-text)] whitespace-nowrap">
                    {row.promptTokens.toLocaleString()} <span className="opacity-50">→</span> {row.completionTokens.toLocaleString()}
                  </td>
                  <td className="px-2 py-1 text-right font-mono text-[var(--color-text-muted)]">{(row.durationMs / 1000).toFixed(1)}s</td>
