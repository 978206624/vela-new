...
import { ChevronDown, ChevronRight, Globe, FolderOpen, RotateCcw, AlertTriangle, Lock, Check } from 'lucide-react'
import {
  BUILTIN_PROMPTS,
  EDITABLE_PROMPT_KEYS,
  CREATIVE_PROMPT_KEYS,
  ANTI_AI_CHECKLIST_ITEMS,
  getPromptTemplate,
  getPromptSource,
  saveCustomPrompt,
...
export default function PromptSettings() {
  ...
  // 挂载时兜底加载全局覆盖（防 App 启动加载晚于设置面板打开的竞态），完成后刷新来源 badge
  useEffect(() => {
    loadCustomPrompts().then(() => setRefreshKey((k) => k + 1))
  }, [])
...
function TemplateItem({
  ...
  // 创作类正文模板：强制注入反 AI 味规则，在系统约束区可见且不可编辑
  const isCreative = CREATIVE_PROMPT_KEYS.includes(builtinTemplate.key)
      ...
      {isExpanded && (
        <div className="px-4 pb-4 space-y-3" style={{ borderTop: '1px solid var(--color-border)' }}>
          ...
          >
            ...
            <p className="text-[0.68rem] leading-relaxed pl-[18px]" style={{ color: 'var(--color-text-muted)' }}>
              {isCreative
                ? '输出格式：纯文本正文，不带 Markdown / 解释 / 思维链；以下规则会随每次生成自动拼装，自定义内容无法移除。'
                : '输出格式约束（如 JSON schema / 纯文本规则）会自动追加到提示词末尾，不受自定义影响。'}
            </p>
            {isCreative && (
              <div className="pl-[18px] mt-2">
                <p className="text-[0.68rem] font-medium mb-1.5" style={{ color: 'var(--color-text-secondary)' }}>
                  反 AI 味规则（Vela 强制注入）
                </p>
                <ul className="space-y-1">
                  {ANTI_AI_CHECKLIST_ITEMS.map((item, i) => (
                    <li key={i} className="flex items-start gap-1.5 text-[0.68rem] leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
                      <Check size={12} style={{ color: 'var(--color-success)', flexShrink: 0, marginTop: 2 }} />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
