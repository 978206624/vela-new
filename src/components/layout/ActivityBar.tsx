...
export default function ActivityBar() {
  ...
  /** 右键显示最近项目上下文菜单 */
  const handleRecentContextMenu = (e: React.MouseEvent, project: { name: string; path: string }) => {
    e.stopPropagation()
    e.preventDefault()
    setRecentContextMenu({ x: e.clientX, y: e.clientY, project })
  }
  ...
  return (
    ...
    >
                    ...
                    {recentProjects
                      ...
                      .map((p, i) => (
                        <button
                          ...
                          onContextMenu={(e) => handleRecentContextMenu(e, p)}
                          className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-left transition-colors hover:bg-[var(--color-hover)]"
                        >
      ...
      {/* 最近项目右键菜单 —— 复用通用 ContextMenu（自带边界钳制 + 点击外部/Esc 关闭） */}
      {recentContextMenu && (
        <ContextMenu
          ...
          items={[
            {
              ...
              onClick: () => ipc.revealInExplorer(recentContextMenu.project.path),
            },
            {
              key: 'remove',
              label: '移除',
              ...
              danger: true,
              onClick: () => useProjectStore.getState().removeRecentProject(recentContextMenu.project.path),
            },
          ]}
