...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 全局级覆盖既不在设置里显示「全局」badge，也不会应用到实际生成
    import('./services/prompt-templates').then(({ loadCustomPrompts }) => {
      loadCustomPrompts()
    }).catch(e => console.warn('[Prompts] 全局自定义覆盖加载失败:', e))
    if (ipc.isElectron) {
    ...
    <div className="flex flex-col w-full h-full overflow-hidden">
      ...
      */}
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* 左侧工具窗口栏（全高，包括底部面板区域） */}
        <div style={{ width: 'var(--width-left-bar)', flexShrink: 0, flexGrow: 0 }}>
          <LeftToolWindowBar />
        </div>

        {/* 横向 PanelGroup：侧边栏 | 中间列（编辑区 + 底部面板）| AI 面板 */}
        <PanelGroup orientation="horizontal" className="flex-1">

          {/* 左侧边栏 */}
          {sidebarOpen && (
            <>
              <Panel id="sidebar" defaultSize="20%" minSize="200px">
          ...
          {/* 中间列：编辑区 + 底部面板（纵向分割） */}
          <Panel id="center" defaultSize="60%" minSize="10%">
          ...
          {aiPanelOpen && (
            <>
              <PanelResizeHandle />
              <Panel id="ai-panel" defaultSize="20%" minSize="200px">
        ...
        {/* 右侧工具窗口栏（全高，包括底部面板区域） */}
        <div style={{ width: 'var(--width-right-bar)', flexShrink: 0, flexGrow: 0 }}>
          <RightToolWindowBar />
        </div>
      </div>
