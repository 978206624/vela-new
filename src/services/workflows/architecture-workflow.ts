  ...
  return [
    {
      ...
      executor: async (cb) => {
        ...
        // 兼容两种返回形态：裸数组 [...]，或被包进对象 { characters/roles/data: [...] }。
        // 原因：请求用了 responseFormat:json_object，而 OpenAI 该模式强制顶层为对象、不能是数组，
        // 模型会把模板要求的数组包成对象返回；旧代码按数组 for...of 遍历对象会直接抛错。
        const parsed = JSON.parse(jsonStr) as unknown
        // 归一为角色数组：裸数组直接用；对象优先取常见键（characters/characterCards/roles/data），
        // 再回退到首个数组值；null 或无数组则空。（null 守卫避免 Object.values(null) 崩溃）
        let parsedCards: Array<Record<string, unknown>> = []
        if (Array.isArray(parsed)) {
          parsedCards = parsed as Array<Record<string, unknown>>
        } else if (parsed && typeof parsed === 'object') {
          const obj = parsed as Record<string, unknown>
          const candidate = obj.characters ?? obj.characterCards ?? obj.roles ?? obj.data
