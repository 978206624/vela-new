...
export function buildAgentSystemPrompt(mode: AgentMode): string {
  ...
  // 4. 工具使用说明（工具能力经原生 function calling 通道传递，此处仅作行为提示）
  const toolPrompt = buildToolAwarenessPrompt()
 ...
 * 工具使用提示（轻量）。
 *
 * 工具的名称/描述/参数 schema 已通过模型原生 function calling 通道传入，
 * 此处不再重复 XML 调用格式，仅提示行为规范（哪些自动执行、哪些需确认）。
 */
function buildToolAwarenessPrompt(): string {
  const tools = toolRegistry.listAll()
  if (tools.length === 0) return ''

  const readonly = tools.filter(t => !t.requiresConfirmation).map(t => t.userFacingName ?? t.name)
  const writeTools = tools.filter(t => t.requiresConfirmation).map(t => t.userFacingName ?? t.name)

  const lines: string[] = [
    '## 工具使用',
    '你已获得以下工具，可按需直接调用（无需任何特殊文本格式，系统会自动执行并把结果回传给你）：',
  ]
  if (readonly.length > 0) lines.push(`- 只读工具（自动执行）：${readonly.join('、')}`)
  if (writeTools.length > 0) lines.push(`- 写入型工具（执行前需用户确认）：${writeTools.join('、')}`)
  lines.push('', '调用工具后，请根据返回结果继续推理并回答用户。需要多步信息时可连续调用多个工具；不要复述工具调用本身。')
