...
export const ipc = {
  ...
  // ===== 窗口控制 =====
  /** 最小化窗口 */
  minimizeWindow: () => { getAPI().invoke('window:minimize') },
  /** 最大化/还原窗口 */
  maximizeWindow: () => { getAPI().invoke('window:maximize') },
  /** 关闭窗口 */
  closeWindow: () => { getAPI().invoke('window:close') },
  /** 查询窗口是否已最大化 */
  isWindowMaximized: () => { return getAPI().invoke('window:is-maximized') as Promise<boolean> },

  // ===== 系统 =====
  /** 在文件管理器中显示 */
  revealInExplorer: (filePath: string) => { getAPI().invoke('system:reveal-in-explorer', filePath) },
}
