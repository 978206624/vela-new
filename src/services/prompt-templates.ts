...
export const EDITABLE_PROMPT_KEYS: string[] = [
  ...
  'world_building',
  'synopsis',
  // 分卷（Phase 19）。两者的 JSON 输出契约都放在 systemSuffix——
  // buildSystemConstraints 恒从 BUILTIN_PROMPTS 取 suffix，用户改不动，
  // 与 PromptSettings.tsx 对用户的承诺（「输出格式约束会自动追加，不受自定义影响」）一致。
...
/**
 * 反 AI 味 checklist 条目（用于 UI 系统约束区的绿勾展示，对应设计屏 26）
 * 五个维度：禁套话 / 对话口语化 / 避免排比堆砌 / 场景具体化 / 节奏起伏
 */
export const ANTI_AI_CHECKLIST_ITEMS: string[] = [
  '禁用「不算/仿佛/这一刻/无论如何/不由得/缓缓/淡淡/嘴角勾起」等 AI 套话词；段尾禁止「他知道，这一切才刚刚开始」「命运的齿轮开始转动」式总结句',
  '对话口语化：贴合人物身份与情绪，不同角色语气有辨识度，避免书面腔与解释性旁白',
  '避免排比堆砌：拒绝三连排比与空洞抒情，形容词慎用',
  '场景与动作具体化：用可感知的动作、神态、五感细节推进，少用抽象概括与上帝视角陈述',
  '段落长短交错，节奏有起伏，张弛有度，拒绝流水账',
]

/**
 * 反 AI 味系统约束正文（强制注入到所有创作类正文模板的 systemSuffix 之后）
 * 始终从此常量取，用户不可覆盖。单一数据源，与 UI checklist 同源。
 */
export const ANTI_AI_CHECKLIST: string =
  '【反 AI 味规则 · Vela 强制注入 · 不可覆盖】\n' +
  ANTI_AI_CHECKLIST_ITEMS.map((s) => `- ${s}`).join('\n')

/** 适用反 AI 味强制约束的创作类（正文生成/精修）模板 key */
export const CREATIVE_PROMPT_KEYS: string[] = [
  'first_chapter_draft',
  'next_chapter_draft',
  'refine_chapter',
  'refine_from_review',
  ...
  "blueprints": [
    {
      "chapterNumber": 1,
      "title": "引人入胜的标题",
      "role": "本章叙事定位（从 建置/铺垫/发展/冲突/高潮/转折/收尾 中选最贴切的一项）",
...
要求：
- **每章必须给出 role**：按叙事节奏从 建置/铺垫/发展/冲突/高潮/转折/收尾 七项中为该章选最贴切的一项（开局建置、蓄势铺垫、推进发展、升级冲突、爆发高潮、扭转转折、收束收尾）；全书结构要有起伏，切忌每章都填「发展」。
...
{{pacing_guidance}}`,
...

  {
    ...
    variables: {
      ...
      genre: '小说类型',
      pacing_guidance: '节奏/风格指导（可选）',
      volume_context: '本卷主线与上一卷收卷状态（单卷模式为空）',
      open_threads: '待回收的未回收伏笔清单（单卷模式为空）',
    },
...
2. 伏笔强制回收与释放：**先对照上面的【前置剧情进度】逐条核对【待回收伏笔台账】**——已经回收、或已在前置章节里安排了回收的条目，本次不得重复处理；剩余条目中标记为「高」的，须在**本卷范围内**安排回收或实质推进（不必挤在本批次）。台账为空时，则对前面章节留下的危机就近引爆或解决。
3. 拒绝水文：每一章都必须有实质性进展。
...
要求：
- **每章必须给出 role**：按叙事节奏从 建置/铺垫/发展/冲突/高潮/转折/收尾 七项中为该章选最贴切的一项，承接前文进度让节奏有起伏，切忌每章都填「发展」。
- 严格遵循上下文连贯，不要前后矛盾。
- 仅给出最终的 JSON 文本，不要解释。
...
{{pacing_guidance}}`,
  ...
  {
    key: 'volume_closing_report',
    name: '卷收束提炼',
    description: '从上一卷的实际写作结果提炼收卷状态与未回收伏笔清单，作为续写下一卷的推演依据',
    systemRole: '你是一位擅长梳理长篇小说线索的故事编辑，善于从已写正文中辨认哪些线索已经闭环、哪些仍然悬而未决。',
    variables: {
      volume_title: '上一卷卷名',
      chapter_notes: '该卷各章的实际写作要点（定稿后处理从正文提炼，非当初的写作计划）',
      character_states: '全书角色当前状态档案',
      prev_open_threads: '该卷开卷时继承的未回收伏笔（首卷为空）',
    },
    content: `请依据【{{volume_title}}】的**实际写作结果**，提炼本卷的收束状态与仍未回收的伏笔。
...
【判断守则】
1. **不得臆造未出现过的线索。** 你看不到全部正文，凡是要点里没提到的线索，**一律不得新增**到清单里。（本条只约束「新增」，不约束下面第 3 条的继承条目。）
2. **已在正文中明确闭环的线索不得列入未回收伏笔。** 判断标准是「读者已经得到答案」，而不是「作者可能还想再写」。
3. **【本卷开卷时继承的未回收伏笔】是既成事实，默认全部保留。** 只有当本卷章节要点**明确证明**它已经闭环时才移出清单；要点里没提到、或无法判断的，**必须原样保留**并沿用其原始埋设章号与优先级。
   ——不要因为「本卷要点没写到它」就当它不存在：那只说明本卷没推进它，不说明它已经解决。漏掉一条，它在后续所有卷里都不会再出现。
4. 收束状态要写「本卷结束时的既有格局」——世界局势、主角处境、关键人物关系与去向、悬而未决的对立，供下一卷直接承接。写成陈述事实的段落，不要写成剧情梗概或宣传语。`,
    systemSuffix: `【输出格式规定】
严格且仅按以下 JSON 格式输出：

{
  "closingState": "本卷结束时的世界格局与主角处境，200-400 字",
  "openThreads": [
    {
      "chapter": 12,
      "thread": "该伏笔的具体内容，一句话说清悬在哪里",
      "urgency": "high"
    }
  ]
}
...
要求：
- **chapter 必须是 JSON 数字类型的 ≥1 整数**——写 \`12\`，不要写 \`"12"\`，不要写「第12章」。取该伏笔最初埋设的章号（不是最近提及的章号）；写不出确切章号的线索宁可不列。
- **urgency 只能是 "high" / "mid" / "low" 三者之一，原样小写英文输出**，不得使用中文「高/中/低」或其它词。high = 读者已明确期待答案、拖下去会失望；mid = 有牵引力但可缓；low = 氛围性线索。
- 单条 thread 控制在 100 字以内（这些内容后续要拼进正文生成的上下文，太长会挤占预算）。整份清单不超过 200 条。
- 没有未回收伏笔时，openThreads 返回空数组 \`[]\`，不要编造。
- 仅给出最终的 JSON 文本，不要解释。`,
  },

  {
    key: 'volume_synopsis',
    name: '下一卷大纲',
    description: '基于上一卷的实际收束结果推演下一卷的主线与卷内大纲，用于原大纲写完后承接续写',
    systemRole: '你是一位经验丰富的网文架构师，擅长在既有格局之上开出新的故事阶段，而不是重启一个新故事。',
    variables: {
      premise: '全书故事前提',
      worldbuilding: '全书世界观设定',
      characters_arch: '全书人物群像网络',
      prev_closing_state: '上一卷的收束状态',
      open_threads: '结转到本卷的未回收伏笔清单',
      user_intent: '作者对本卷的意图（可为空，空则完全由 AI 推演）',
...
2. **旧线索要有交代**：结转伏笔中标记 high 的，应在本卷内安排回收或实质推进；mid / low 可继续悬置，但不要遗忘。
3. **本卷要有独立的完整性**：有自己的主线目标、核心冲突与阶段性高潮，读完这一卷读者能获得阶段性满足，同时留下通向下一卷的引子。不要写成「过渡卷」。
4. **不是全书收尾**：这是长篇的一个阶段，不要在本卷终结全书主线，也不要写大结局式的收束。
5. 若【作者对本卷的意图】非空，它的优先级高于你自己的推演；意图与既有设定冲突时，以既有设定为准并在大纲里给出自洽的衔接方式。
...
{
  "title": "第N卷 · 卷名",
  "premise": "本卷主线目标 + 核心冲突，150-300 字",
  "synopsis": "本卷大纲，按结构指导分段展开，每段标明覆盖章号区间",
  "suggestedChapterCount": 60
}

要求：
- title 用「第N卷 · 卷名」格式，卷名 4-8 字、贴合本卷主题。
...
2. 分析并在 \`newCharacters\` 中提取本章新出场的重要角色（不要包含路人或已死无后续影响的龙套）。其 \`role\` 字段**仅限以下四个英文枚举之一**（与角色卡「定位」一致，不要用其它中文或自由描述）：protagonist（主角）、antagonist（反派）、supporting（配角）、minor（龙套）。
3. \`currentState\` 字段说明：
...
如果图谱中没有任何可提取的角色，返回空数组 []。`,
  ...
  // ================================================================
  // AI 一键补全角色人设 — 从出场正文反向推断静态人设
  // ================================================================
  {
    key: 'infer_character_profile',
    name: 'AI 补全角色人设',
    description: '从角色出场的定稿正文片段 + 架构人物群像，反向推断角色的静态人设字段，回填空字段',
    systemRole: '你是一位资深的小说人物分析师，擅长从正文叙事中精准提炼角色的外貌、性格、背景等设定，只依据文本既有信息归纳，绝不编造。',
    variables: {
      character_name: '目标角色名',
      character_dynamics: '架构「人物群像」设定（可能为空）',
      appearance_excerpts: '该角色出场的定稿正文片段（已按出场聚合）',
    },
    content: `请基于以下材料，为角色「{{character_name}}」归纳其静态人设。
...
【任务要求】
1. 只为「{{character_name}}」这一个角色归纳，不要混入其他角色的信息。
2. 严格只依据上面材料中**已有的信息**归纳，材料中没有提及的字段，对应 profile 值留**空字符串**——**绝对不要编造、不要脑补**。
3. 需要归纳的 7 个静态字段：appearance（外貌）、personality（性格）、background（背景）、abilities（能力/技能/修为）、motivation（核心动机）、arc（成长弧光）、relationships（与其他角色的关系）。
4. 每个**非空**字段，在 evidence 里给出支撑它的章节号与原文摘录（quote 取材料中的关键句，便于用户核对）；留空的字段不需要 evidence。
...
【输出格式（严格 JSON，两块分离）】
{
  ...
  "evidence": {
    "appearance": [{ "chapter": 1, "quote": "支撑该字段的原文摘录" }]
  }
}

profile 的 7 个字段值必须是字符串；evidence 仅对有内容的字段给出、仅供用户核对，不写入角色卡。`,
  },
...
3. role 必须从 建置/铺垫/发展/冲突/高潮/转折/收尾 七项中选最贴切的一项（按正文叙事功能判断）。
4. 仅输出 JSON，不要任何额外文字。`,
  },
...
export async function loadCustomPrompts(): Promise<void> {
  try {
    ...
    customPrompts.clear() // 重载前清空：避免外部删除文件后残留旧缓存，或 HMR 重复调用累积
...
export async function saveCustomPrompt(template: PromptTemplate): Promise<boolean> {
  try {
    ...
    // 标记已加载：否则 getPromptSource/getPromptTemplate 的全局分支被门控跳过，
    // 导致刚保存的全局覆盖既不显示也不生效（与启动期 loadCustomPrompts 双保险）
    customPromptsLoaded = true
    return true
  } catch {
    return false
...
/**
 * 解析模板的 system role：内置 key 始终从 BUILTIN_PROMPTS 取——
 * 系统角色与 systemSuffix 同属系统约束，不可被用户自定义覆盖（防经手写 JSON 篡改角色弱化反 AI）。
 * 未知 key（纯自定义模板）回退传入值。
 */
export function resolveSystemRole(key: string, fallback?: string): string {
  const builtinTemplate = BUILTIN_PROMPTS.find((p) => p.key === key)
  return builtinTemplate?.systemRole ?? fallback ?? ''
}

/**
 * 完成提示词最终拼装：追加系统约束（始终从内置取，创作类含反 AI 味）+ 裁剪空段落。
 * 供 renderPrompt 与 BasePromptBuilder.build() 共用，保证「预览」与「执行」逻辑完全镜像。
 */
export function finalizePrompt(content: string, key: string, variables: Record<string, string>): string {
